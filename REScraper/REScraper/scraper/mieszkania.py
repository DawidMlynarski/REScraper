...
def scrape_page(page_number,location,transaction):
    ...
    data = [(title.text.strip(), price.text.strip(), location_of_offer.text.strip()) for title, price,location_of_offer in zip(titles, prices,location_of_offer)]
    return data
...
def scrape_multiple_pages(start_page, end_page,location,price,keyword,area,price_per_sqm,sorting,transaction):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(future_to_page): 
            ...
            try:
                data = future.result()
                for title, price,location in data:
                    offers_data.append((title, price,location))
